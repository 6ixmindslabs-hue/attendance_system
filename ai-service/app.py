...
GUIDE_MIN_FACE_AREA_RATIO = float(os.environ.get("FACE_GUIDE_MIN_AREA_RATIO", "0.035"))
...
ENABLE_CAPTURE_RECOVERY = os.environ.get("ENABLE_CAPTURE_RECOVERY", "true").strip().lower() != "false"
RECOVERY_MIN_BRIGHTNESS = float(os.environ.get("FACE_RECOVERY_MIN_BRIGHTNESS", "0.16"))
RECOVERY_MIN_BLUR_VARIANCE = float(os.environ.get("FACE_RECOVERY_MIN_BLUR_VARIANCE", "22.0"))
RECOVERY_TARGET_BRIGHTNESS = float(os.environ.get("FACE_RECOVERY_TARGET_BRIGHTNESS", "0.34"))
RECOVERY_MIN_QUALITY_SCORE = float(os.environ.get("FACE_RECOVERY_MIN_QUALITY_SCORE", "0.18"))
RECOVERY_CLAHE_CLIP_LIMIT = float(os.environ.get("FACE_RECOVERY_CLAHE_CLIP_LIMIT", "2.5"))
RECOVERY_CLAHE_TILE_GRID = max(2, int(os.environ.get("FACE_RECOVERY_CLAHE_TILE_GRID", "8")))
RECOVERY_UNSHARP_SIGMA = float(os.environ.get("FACE_RECOVERY_UNSHARP_SIGMA", "1.4"))
RECOVERY_UNSHARP_AMOUNT = float(os.environ.get("FACE_RECOVERY_UNSHARP_AMOUNT", "1.15"))
...
RECOVERABLE_REASON_CODES = {"no_face", "low_brightness", "low_sharpness"}
...
def apply_gamma_correction(image: np.ndarray, gamma: float) -> np.ndarray:
    safe_gamma = float(np.clip(gamma, 0.45, 1.0))
    normalized = image.astype(np.float32) / 255.0
    corrected = np.power(normalized, safe_gamma)
    return np.clip(corrected * 255.0, 0.0, 255.0).astype(np.uint8)


def apply_clahe_to_luminance(image: np.ndarray) -> np.ndarray:
    lab = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
    luminance, channel_a, channel_b = cv2.split(lab)
    clahe = cv2.createCLAHE(
        clipLimit=RECOVERY_CLAHE_CLIP_LIMIT,
        tileGridSize=(RECOVERY_CLAHE_TILE_GRID, RECOVERY_CLAHE_TILE_GRID)
    )
    enhanced_luminance = clahe.apply(luminance)
    return cv2.cvtColor(cv2.merge((enhanced_luminance, channel_a, channel_b)), cv2.COLOR_LAB2BGR)


def apply_unsharp_mask(image: np.ndarray) -> np.ndarray:
    blurred = cv2.GaussianBlur(image, (0, 0), sigmaX=RECOVERY_UNSHARP_SIGMA, sigmaY=RECOVERY_UNSHARP_SIGMA)
    return cv2.addWeighted(
        image,
        1.0 + RECOVERY_UNSHARP_AMOUNT,
        blurred,
        -RECOVERY_UNSHARP_AMOUNT,
        0
    )


def improve_low_light_image(image: np.ndarray, source_brightness: Optional[float] = None) -> np.ndarray:
    current_brightness = source_brightness if source_brightness is not None else get_image_brightness(image)
    gamma = float(np.clip(current_brightness / max(RECOVERY_TARGET_BRIGHTNESS, 1e-6), 0.55, 0.95))
    gamma_corrected = apply_gamma_correction(image, gamma)
    return apply_clahe_to_luminance(gamma_corrected)
...
def detect_single_face(image: np.ndarray, enforce_guide: bool = False) -> Dict[str, Any]:
    if enforce_guide:
        ...
        return {
            "accepted": True,
            "face": guide_faces[0],
            "guide_distance": compute_guide_distance(image, guide_faces[0])
        }
...
def compute_face_quality(image: np.ndarray, face: Any, guide_distance: Optional[float] = None) -> Dict[str, float]:
    ...
    min_face_area_ratio = GUIDE_MIN_FACE_AREA_RATIO if guide_distance is not None else MIN_FACE_AREA_RATIO
...
def evaluate_face_capture(image: np.ndarray, enforce_guide: bool = False, recovery_mode: bool = False, variant: str = "raw") -> Dict[str, Any]:
    detection = detect_single_face(image, enforce_guide=enforce_guide)

    if not detection["accepted"]:
        detection["variant"] = variant
        detection["recovery_mode"] = recovery_mode
        return detection

    face = detection["face"]
    guide_distance = detection.get("guide_distance")
    metrics = compute_face_quality(image, face, guide_distance=guide_distance)
    min_brightness = RECOVERY_MIN_BRIGHTNESS if recovery_mode else MIN_BRIGHTNESS
    min_blur_variance = RECOVERY_MIN_BLUR_VARIANCE if recovery_mode else MIN_BLUR_VARIANCE
    ...
    if metrics["quality_score"] < RECOVERY_MIN_QUALITY_SCORE and recovery_mode:
        return {
            "accepted": False,
            "reason": "Image quality is still too low for reliable recognition.",
    ...
    if metrics["face_area_ratio"] < min_face_area_ratio:
        return {
            "accepted": False,
            "reason": "Face is too small in the frame.",
    ...
    if not enforce_guide and metrics["center_offset"] > MAX_CENTER_OFFSET:
        return {
            "accepted": False,
            "reason": "Face is not centered clearly enough.",
            "reason_code": "face_not_centered",
            "metrics": metrics,
            "variant": variant,
            "recovery_mode": recovery_mode
        }
...
def should_try_capture_recovery(result: Dict[str, Any]) -> bool:
    if not ENABLE_CAPTURE_RECOVERY:
        return False

    if not result.get("accepted"):
        return result.get("reason_code") in RECOVERABLE_REASON_CODES

    metrics = result.get("metrics") or {}
    return (
        float(metrics.get("brightness", 1.0)) < RECOVERY_TARGET_BRIGHTNESS or
        float(metrics.get("blur_variance", MIN_BLUR_VARIANCE * 2.0)) < (MIN_BLUR_VARIANCE * 1.35)
    )


def build_recovery_variants(image: np.ndarray, baseline_result: Dict[str, Any]) -> List[tuple[str, np.ndarray]]:
    reason_code = baseline_result.get("reason_code")
    metrics = baseline_result.get("metrics") or {}
    source_brightness = float(metrics.get("brightness", get_image_brightness(image)))
    ...
    def add_variant(label: str, variant_image: np.ndarray) -> None:
        ...
        variants.append((label, variant_image))

    if source_brightness < RECOVERY_TARGET_BRIGHTNESS or reason_code in {"no_face", "low_brightness"}:
        low_light_variant = improve_low_light_image(image, source_brightness=source_brightness)
        add_variant("low_light", low_light_variant)
        add_variant("low_light_sharpened", apply_unsharp_mask(low_light_variant))

    if reason_code in {"no_face", "low_sharpness"} or float(metrics.get("blur_variance", 0.0)) < (MIN_BLUR_VARIANCE * 1.35):
        sharpened_variant = apply_unsharp_mask(image)
        add_variant("sharpened", sharpened_variant)

    return variants


def combine_capture_embeddings(results: List[Dict[str, Any]]) -> np.ndarray:
    if len(results) == 1:
        return normalize_embedding(results[0]["embedding"])

    weights = np.array([max(float(result.get("quality_score", 0.0)), 0.15) for result in results], dtype=np.float32)
    embeddings = np.stack([normalize_embedding(result["embedding"]) for result in results], axis=0)
    combined = np.average(embeddings, axis=0, weights=weights)
    return normalize_embedding(combined)


def get_result_rank(result: Dict[str, Any]) -> tuple[float, float, float, float]:
    metrics = result.get("metrics") or {}
    return (
        1.0 if result.get("accepted") else 0.0,
        float(metrics.get("quality_score", 0.0)),
        float(metrics.get("brightness", 0.0)),
        float(metrics.get("blur_variance", 0.0))
    )


def validate_face_capture(image: np.ndarray, enforce_guide: bool = False) -> Dict[str, Any]:
    baseline_result = evaluate_face_capture(image, enforce_guide=enforce_guide, recovery_mode=False, variant="raw")

    if not should_try_capture_recovery(baseline_result):
        return baseline_result

    accepted_results = [baseline_result] if baseline_result.get("accepted") else []
    best_result = baseline_result

    for label, variant_image in build_recovery_variants(image, baseline_result):
        variant_result = evaluate_face_capture(
            variant_image,
            enforce_guide=enforce_guide,
            recovery_mode=True,
            variant=label
        )

        if get_result_rank(variant_result) > get_result_rank(best_result):
            best_result = variant_result

        if variant_result.get("accepted"):
            accepted_results.append(variant_result)

    if not accepted_results:
        return best_result

    primary_result = max(accepted_results, key=get_result_rank)
    combined_embedding = combine_capture_embeddings(accepted_results)

    return {
        **primary_result,
        "embedding": combined_embedding,
        "quality_score": max(float(result.get("quality_score", 0.0)) for result in accepted_results),
        "recovery_applied": any(result.get("recovery_mode") for result in accepted_results),
        "accepted_variants": [str(result.get("variant", "raw")) for result in accepted_results]
    }
...
async def recognize(req: RecognizeRequest) -> Dict[str, Any]:
    ...
    capture_metadata = {
        "capture_quality_score": round(float(result.get("quality_score", 0.0)), 6),
        "recovery_applied": bool(result.get("recovery_applied", False)),
        "capture_variants": result.get("accepted_variants") or [str(result.get("variant", "raw"))]
    }
