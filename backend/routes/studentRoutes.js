...
import { authenticateRequest, requireAdmin, requireStudentSelf } from '../middleware/authMiddleware.js';
...
router.get('/departments', authenticateRequest, requireAdmin, async (_req, res) => {
    try {
        const { data: departments, error } = await supabase
            .from('departments')
            .select('id, name')
...
router.get('/:studentId', authenticateRequest, requireStudentSelf, async (req, res) => {
    try {
...
router.put('/:studentId/re-enroll-face', authenticateRequest, requireAdmin, async (req, res) => {
    try {
        const { studentId } = req.params;
