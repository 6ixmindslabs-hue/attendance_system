...
export const isAfternoonPeriod = (value = '') => {
    const normalizedValue = String(value || '').trim().toLowerCase();
    return normalizedValue === 'afternoon' || normalizedValue === 'evening';
};

export const formatAttendancePeriodLabel = (value = '') => {
...
const validateAttendanceSettings = (settings) => {
    ...
    if (toMinutes(settings.morning_end) > toMinutes(settings.evening_start)) {
        throw new Error('Morning attendance must end before the afternoon window starts.');
    }
